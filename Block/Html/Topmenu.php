 ...
 */
namespace RocktechnolabsDev\CategoryAttr\Block\Html;
...
{
    public const CATEGORY_TREE_PREFIX = 'category-node-';
    ...
    protected function _getHtml(
        ...
        foreach ($children as $child) {
            ...
            if (strpos($child->getId(), self::CATEGORY_TREE_PREFIX) !== false) {
                $categoryId = explode(self::CATEGORY_TREE_PREFIX, $child->getId());
                $categoryId = array_filter($categoryId);
            }

            $categoryMenuImage = $menuImageHtml = '';
            if (isset($categoryId[1])) {
                $categoryMenuImage = $this->getCategoryMenuImage($categoryId[1]);
            }

            if ($childLevel == 1 && $categoryMenuImage) {
                $menuImageHtml = '<img src="'. $categoryMenuImage.'" title="'.$this->escapeHtml($child->getName()).'"
                    alt="'.$this->escapeHtml($child->getName()).'" />';
            }

            $html .= '<li ' . $this->_getRenderedMenuItemAttributes($child) . '>';
            $html .= '<a href="' . $child->getUrl() . '" ' . $outermostClassCode . '><span>'.$this->escapeHtml(
                $child->getName()
            ) . '</span>'.$menuImageHtml.'</a>' . $this->_addSubMenu(
                $child,
                $childLevel,
                $childrenWrapClass,
                $limit
    ...
    /**
     * Get category menu image
     *
     * @param int $categoryId
     * @return bool|string
     */
    public function getCategoryMenuImage($categoryId)
    {
        $categoryMenuImage = $categoryMenuImageRelativePath = '';
        $categoryInfo = $this->categoryFactory->create()->load($categoryId);
        if ($categoryInfo && $categoryInfo->getId()) {
            $categoryMenuImageRelativePath = $categoryInfo->getData('menu_image');
            if ($categoryMenuImageRelativePath) {
                $mediaDir = $this->storeManager->getStore()->getBaseUrl();
                $categoryMenuImage = $mediaDir.$categoryMenuImageRelativePath;
